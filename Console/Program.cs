...
namespace Console
{
    class Program
    {
        ...
        {
            Regex hyperLinkAbsoluteUriRegex = new Regex("<\\s*(?<Tag>(a|base|form|frame))\\s*.*?(?<AttributeName>(action|href|src))\\s*=\\s*([\\\"\\'])(?<HyperLink>.*?)\\3",
                    ...
                    while (_absoluteUris.Count != 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (_assignHyperLinkDiscoveries)
                            {
                                ...
                                string decodedHtml = WebUtility.HtmlDecode(Encoding.Default.GetString(data));
